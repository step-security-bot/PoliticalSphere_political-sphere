...
if [[ "${SKIP_AI_NETWORK_TESTS}" == "1" ]]; then
  echo "Skipping index server smoke tests (SKIP_AI_NETWORK_TESTS=1)"
else
  # Test index server (start/stop quickly)
  echo "Testing index server..."
  AI_INDEX_HOST=127.0.0.1 node "$SCRIPT_DIR/index-server.js" &
  SERVER_PID=$!
  sleep 3
  ...
  SEARCH_RESPONSE=$(curl -s "http://localhost:3001/search?q=function" || true)
  ...
  INVALID_RESPONSE=$(curl -s "http://localhost:3001/search?q=" || true)
  ...
  METRICS_RESPONSE=$(curl -s "http://localhost:3001/metrics" || true)
