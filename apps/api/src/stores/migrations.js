...
const getDbPath = () => {
  // Use repo root as base path for DB file. This avoids issues with
  // __dirname not being available in pure ESM and keeps tests stable
  // by defaulting to the repository working directory.
  return path.join(process.cwd(), 'data', 'runtime', 'political_sphere.db');
};

let DB_PATH = getDbPath();

// Export for debugging (DB_PATH is also exported below)

// For test runs, use a test database file to ensure Prisma and better-sqlite3 use the same database
...
function initializeDatabase() {
  ...
  info('🗄️  Initializing database at path:', DB_PATH);
...
export { DB_PATH, initializeDatabase, runMigrations };
