...
import { info, error as logError, warn } from './logger.cjs';
import { DB_PATH, DEFAULT_DB_PATH } from '../config.cjs';
import {
  MigrationError,
  MigrationRollbackError,
  MigrationValidationError,
} from './migration-error.cjs';
...
async function loadMigrations() {
  const migrationsDir = import.meta.dirname || path.dirname(new URL(import.meta.url).pathname);
  const files = fs
    .readdirSync(migrationsDir)
    .filter(file => file.endsWith('.mjs') && file !== 'index.mjs' && file !== 'migration-error.cjs')
...
export { initializeDatabase, runMigrations, rollbackAllMigrations };
